...
# AnoSecureKit Compatibility Fixtures

These files pin v1 wire-format compatibility for AnoSecureKit.
...
These are AnoSecureKit v1 known-answer vectors, not external standard vectors.
They were generated from fixed keys, nonces, passwords, plaintext, AAD, and
format parameters listed below, then committed as lowercase serialized hex. The
`SKF1` raw-key fixtures use HKDF context label `anosecurekit file sealing v1`.
...
- `skt1-aes256-gcm-aad.hex`: `SKT1`, key `00..00`, nonce `00..00`, plaintext `hello anosecurekit`, AAD `record:v1`.
