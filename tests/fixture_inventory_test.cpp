...
{
	return std::filesystem::path(ANOSECUREKIT_TEST_FIXTURE_DIR);
}

std::filesystem::path format_doc()
{
	return std::filesystem::path(ANOSECUREKIT_FORMAT_DOC);
}
...
std::uint32_t read_be32(const anosecurekit::bytes &bytes, std::size_t offset)
{
...
TEST(FixtureInventory, Skt1FixturesUseDocumentedHeader)
{
	...
	for (const std::string &name : names)
	{
		...
		SCOPED_TRACE(name);
		const anosecurekit::bytes fixture = anosecurekit::test::read_hex_fixture(name);
