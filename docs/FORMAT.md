...
This document is the compatibility reference for AnoSecureKit serialized data.
...
AnoSecureKit does not expose caller-selected algorithms, caller-selected nonces, or
...
`SKT1` is produced by `anosecurekit::encrypt`, `anosecurekit::wrap_key`, and
`packet_encryptor`.
...
`SKF1` is produced by `anosecurekit::seal_file` with a caller-provided 32-byte key.
The caller key is not used directly for chunks. AnoSecureKit derives a per-file key
with HKDF-SHA-256, a random file salt, and the exact context label
`anosecurekit file sealing v1`. The context label is part of the v1
compatibility contract and must not change under the same `SKF1` magic/version
pair. If that label changes, treat it as a byte-format change and update this
document, fixtures, and tests in the same change.
...
`SKP1` is produced by `anosecurekit::seal_file_with_password`. It uses the same
chunk record layout as `SKF1`, but derives the per-file key directly from
caller-provided password bytes with scrypt.
...
Password bytes are accepted exactly as supplied by the caller. AnoSecureKit does
...
succeeds, and stream file open writes to caller-owned output that AnoSecureKit
cannot roll back.
