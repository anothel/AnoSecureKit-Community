...
anosecurekit::bytes make_bytes(std::size_t size)
{
...
try
{
	const anosecurekit::bytes payload = make_bytes(64u * 1024u);
	const anosecurekit::bytes file_payload = make_bytes(2u * 1024u * 1024u);
	const anosecurekit::bytes aad = make_bytes(32u);
	const anosecurekit::key256 key = fixed_key();

	std::cout << "anosecurekit benchmark\n";

	print(measure("sha256_64k", 128u, payload.size() * 128u, [&](std::size_t) {
		const std::array<std::byte, 32u> digest = anosecurekit::sha256(payload);
	...
	print(measure("aead_roundtrip_64k", 64u, payload.size() * 64u, [&](std::size_t) {
		const anosecurekit::bytes packet = anosecurekit::encrypt(payload, key, aad);
	...
	const std::filesystem::path root =
	    std::filesystem::temp_directory_path() / ("anosecurekit-bench-" + anosecurekit::random_token(16u));
	...
	print(measure("file_roundtrip_2m", 3u, file_payload.size() * 3u, [&](std::size_t index) {
		...
		anosecurekit::seal_file(input, sealed, key, aad);
		anosecurekit::open_file(sealed, output, key, aad);
		const anosecurekit::bytes opened = read_file(output);
