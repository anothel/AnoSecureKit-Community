...
void reject_invalid_base64(std::string_view input)
{
	if ((input.size() % 4) != 0)
	{
		throw anosecurekit::error(anosecurekit::error_code::invalid_encoding, "base64 input length must be a multiple of four");
	}
	...
	for (std::size_t i = 0; i < input.size(); ++i)
	{
		...
		if (ch == '=')
		{
			...
			if (padding > 2)
			{
				throw anosecurekit::error(anosecurekit::error_code::invalid_encoding, "base64 input has too much padding");
			}
			continue;
		}
		...
		if (seen_padding)
		{
			throw anosecurekit::error(anosecurekit::error_code::invalid_encoding, "base64 input has data after padding");
		}

		if (decode_char(ch) < 0)
		{
			throw anosecurekit::error(anosecurekit::error_code::invalid_encoding, "base64 input contains an invalid character");
		}
	}

	if (padding > 0 && input.size() < 4)
	{
		throw anosecurekit::error(anosecurekit::error_code::invalid_encoding, "base64 padding without data");
	}
}
...
void reject_invalid_base64url(std::string_view input)
{
	if ((input.size() % 4) == 1)
	{
		throw anosecurekit::error(anosecurekit::error_code::invalid_encoding, "base64url input has invalid length");
	}
	...
	for (char ch : input)
	{
		...
		{
			throw anosecurekit::error(anosecurekit::error_code::invalid_encoding, "base64url input contains an invalid character");
		}
	}
}
