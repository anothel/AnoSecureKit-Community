...
void seal_stream_payload(
    ...
    const anosecurekit::key256 &file_key,
    std::span<const std::byte> aad)
{
	write_all(out, serialized_header);

	anosecurekit::bytes buffer(kChunkSize);
	...
	while (!wrote_final)
	{
		...
		const anosecurekit::bytes chunk_aad = make_chunk_aad(serialized_header, record, aad);
		...
		{
			throw anosecurekit::error(anosecurekit::error_code::invalid_input, "file is too large to seal");
		}
		++chunk_index;
	}
}
    ...
    std::span<const std::byte> aad)
{
	...
	while (!saw_final)
	{
		...
		anosecurekit::bytes ciphertext(record.plaintext_size);
		...
		const anosecurekit::bytes chunk_aad = make_chunk_aad(serialized_header, record, aad);
		anosecurekit::bytes plaintext = decrypt_chunk(ciphertext, tag, file_key, nonce, chunk_aad);
		const anosecurekit::internal::wipe_on_exit wipe_plaintext{std::span<std::byte>(plaintext)};
