...
void print_help()
{
	std::cout << "Usage:\n"
	          ...
	          << "  anosecurekit hkdf-sha256 --key-hex <hex> --salt-hex <hex> --info-hex <hex> --out-size "
	             "<byte-size>\n"
	          << "  anosecurekit hkdf-sha256 --key-hex <hex> --salt-hex <hex> --info-text <text> --out-size "
	             "<byte-size>\n"
	          ...
	          << "  anosecurekit wrap-key (--key-hex <64-hex>|--key-file <path>) "
	             "(--wrapping-key-hex <64-hex>|--wrapping-key-file <path>) [--out <path>]\n"
	          << "  anosecurekit unwrap-key (--packet-hex <hex>|--packet-file <path>) "
	             "(--wrapping-key-hex <64-hex>|--wrapping-key-file <path>) [--out <path>]\n"
	          << "  anosecurekit encrypt (--text <text>|--in <path>) (--key-hex <64-hex>|--key-file <path>) "
	             "[--aad-text <text>|--aad-hex <hex>] [--out <path>]\n"
	          << "  anosecurekit decrypt (--packet-hex <hex>|--packet-file <path>) "
	             "(--key-hex <64-hex>|--key-file <path>) [--aad-text <text>|--aad-hex <hex>] [--out <path>]\n"
	          << "  anosecurekit seal-file --in <path|-> --out <path|-> --key-hex <64-hex> "
	             "[--aad-text <text>|--aad-hex <hex>]\n"
	          << "  anosecurekit open-file --in <path|-> --out <path|-> --key-hex <64-hex> "
	             "[--aad-text <text>|--aad-hex <hex>]\n"
	          << "  anosecurekit seal-file --in <path|-> --out <path|-> --key-file <path> "
	             "[--aad-text <text>|--aad-hex <hex>]\n"
	          << "  anosecurekit open-file --in <path|-> --out <path|-> --key-file <path> "
	             "[--aad-text <text>|--aad-hex <hex>]\n"
	          << "  anosecurekit verify-file --in <path|-> --key-hex <64-hex> "
	             "[--aad-text <text>|--aad-hex <hex>]\n"
	          << "  anosecurekit verify-file --in <path|-> --key-file <path> "
	             "[--aad-text <text>|--aad-hex <hex>]\n"
	          << "  anosecurekit seal-file-password --in <path|-> --out <path|-> --password-file <path> "
	             "[--aad-text <text>|--aad-hex <hex>]\n"
	          << "  anosecurekit open-file-password --in <path|-> --out <path|-> --password-file <path> "
	             "[--aad-text <text>|--aad-hex <hex>]\n"
	          << "  anosecurekit verify-file-password --in <path|-> --password-file <path> "
	             "[--aad-text <text>|--aad-hex <hex>]\n"
...
void print_version()
{
	std::cout << "anosecurekit " << anosecurekit::version() << '\n';
}
...
{
	return "Usage:\n  anosecurekit hmac-sha256 --key-hex <hex> --text <text>\n  anosecurekit hmac-sha256 --key-hex "
	       "<hex> --file <path>";
}
...
std::string_view file_command_usage(std::string_view command)
{
	...
	{
		return "Usage:\n  anosecurekit seal-file --in <path|-> --out <path|-> (--key-hex <64-hex>|--key-file <path>) "
		       "[--aad-text <text>|--aad-hex <hex>]";
	}
	if (command == "open-file")
	{
		return "Usage:\n  anosecurekit open-file --in <path|-> --out <path|-> (--key-hex <64-hex>|--key-file <path>) "
		       "[--aad-text <text>|--aad-hex <hex>]";
	}
	return "Usage:\n  anosecurekit verify-file --in <path|-> (--key-hex <64-hex>|--key-file <path>) "
	       "[--aad-text <text>|--aad-hex <hex>]";
}
...
void write_bytes(const anosecurekit::bytes &bytes)
{
...
anosecurekit::key256 key_from_hex(std::string_view text)
{
	if (text.size() != 64)
	{
	...
	const anosecurekit::bytes decoded = anosecurekit::hex_decode(text);
...
anosecurekit::bytes password_from_file(const std::filesystem::path &path)
{
	anosecurekit::bytes password = read_file(path);
	if (password.empty())
	{
...
anosecurekit::key256 key_from_file(const std::filesystem::path &path)
{
	const anosecurekit::bytes key_bytes = read_file(path);
	std::string text;
...
anosecurekit::key256 key_from_option(std::string_view option, std::string_view value)
{
	if (option == "--key-hex")
	{
...
anosecurekit::key256 wrapping_key_from_option(std::string_view option, std::string_view value)
{
...
anosecurekit::bytes aad_from_option(std::string_view option, std::string_view value)
{
	if (option == "--aad-text")
	{
	...
	if (option == "--aad-hex")
	{
		return anosecurekit::hex_decode(value);
	}
...
{
	anosecurekit::key256 key{};
	anosecurekit::key256 wrapping_key{};
	std::filesystem::path output;
	bool has_key = false;
...
void write_key_file(const std::filesystem::path &path, const anosecurekit::key256 &key)
{
	write_new_output_file(path, [&](std::ostream &out) {
		out << anosecurekit::hex_encode(key) << '\n';
	});
}

void write_generated_key(const std::filesystem::path &path)
{
	write_key_file(path, anosecurekit::generate_key());
}
...
std::filesystem::path temporary_output_path_for(const std::filesystem::path &output)
{
	...
	const anosecurekit::bytes token = anosecurekit::random_bytes(16);
	std::string suffix;
	suffix.reserve(45);
	suffix += ".anosecurekit.";
...
unwrap_key_options parse_unwrap_key_options(int argc, char **argv)
{
	...
	for (int index = 2; index < argc; index += 2)
	{
		...
		if (option == "--packet-hex" || option == "--packet-file")
		{
			...
			options.packet = option == "--packet-hex" ? anosecurekit::hex_decode(value) : read_file(std::filesystem::path(value));
...
int run(int argc, char **argv)
{
	try
	{
		...
		{
			std::cout << anosecurekit::random_token(parse_size(argv[2])) << '\n';
			return 0;
		}
		...
		{
			std::cout << anosecurekit::hex_encode(anosecurekit::sha256(bytes_from_text(argv[3]))) << '\n';
			return 0;
		}

		if (argc == 4 && is_arg(argv[1], "sha256") && is_arg(argv[2], "--file"))
		{
			std::cout << anosecurekit::hex_encode(anosecurekit::sha256(read_file(argv[3]))) << '\n';
			return 0;
		}
		...
		if (argc == 6 && is_arg(argv[1], "hmac-sha256") && is_arg(argv[2], "--key-hex") &&
		    is_arg(argv[4], "--text"))
		{
			std::cout << anosecurekit::hex_encode(
			                 anosecurekit::hmac_sha256(anosecurekit::hex_decode(argv[3]), bytes_from_text(argv[5])))
			          << '\n';
			return 0;
		}
		...
		{
			std::cout
			    << anosecurekit::hex_encode(anosecurekit::hmac_sha256(anosecurekit::hex_decode(argv[3]), read_file(argv[5])))
			    << '\n';
			return 0;
		}
		...
		{
			const anosecurekit::bytes info = is_arg(argv[6], "--info-hex") ? anosecurekit::hex_decode(argv[7])
			                                                               : bytes_from_text(argv[7]);
			std::cout << anosecurekit::hex_encode(anosecurekit::hkdf_sha256(
			                 anosecurekit::hex_decode(argv[3]), anosecurekit::hex_decode(argv[5]), info, parse_size(argv[9])))
			          << '\n';
			return 0;
		}
		...
		{
			std::cout << anosecurekit::hex_encode(bytes_from_text(argv[3])) << '\n';
			return 0;
		}
		...
		{
			write_bytes(anosecurekit::hex_decode(argv[3]));
			return 0;
		}
		...
		{
			std::cout << anosecurekit::base64url_encode(bytes_from_text(argv[3])) << '\n';
			return 0;
		}
		...
		{
			write_bytes(anosecurekit::base64url_decode(argv[3]));
			return 0;
		}
		...
		if (argc >= 2 && is_arg(argv[1], "wrap-key"))
		{
			...
			const anosecurekit::bytes packet = anosecurekit::wrap_key(options.key, options.wrapping_key);
			...
			{
				std::cout << anosecurekit::hex_encode(packet) << '\n';
			}
			return 0;
		}
		...
		if (argc >= 2 && is_arg(argv[1], "unwrap-key"))
		{
			...
			const anosecurekit::key256 key = anosecurekit::unwrap_key(options.packet, options.wrapping_key);
			if (options.has_output)
			{
			...
			{
				std::cout << anosecurekit::hex_encode(key) << '\n';
			}
			return 0;
		}
		...
		if (argc >= 2 && is_arg(argv[1], "encrypt"))
		{
			...
			const anosecurekit::bytes packet = anosecurekit::encrypt(options.plaintext, options.key, options.aad);
			if (options.has_output)
			{
				write_binary_file(options.output, packet);
			}
		...
		if (argc >= 2 && is_arg(argv[1], "decrypt"))
		{
			...
			const anosecurekit::bytes plaintext = anosecurekit::decrypt(options.packet, options.key, options.aad);
			if (options.has_output)
			{
		...
		if (argc >= 2 && (is_arg(argv[1], "seal-file") || is_arg(argv[1], "open-file") || is_arg(argv[1], "verify-file")))
		{
			...
			if (is_arg(argv[1], "seal-file"))
			{
				if (uses_standard_stream)
				{
					run_streaming_file_command(options, [&](std::istream &input, std::ostream &output) {
						anosecurekit::seal_file(input, output, options.key, options.aad);
					});
				}
				else
				{
					anosecurekit::seal_file(options.input, options.output, options.key, options.aad);
				}
			}
			else if (is_arg(argv[1], "open-file"))
			{
				...
				{
					anosecurekit::open_file(options.input, options.output, options.key, options.aad);
				}
			}
			else
			{
		                     ...
		                     is_arg(argv[1], "verify-file-password")))
		{
			...
			if (is_arg(argv[1], "seal-file-password"))
			{
				...
				{
					run_streaming_file_command(options, [&](std::istream &input, std::ostream &output) {
						anosecurekit::seal_file_with_password(input, output, options.password, options.aad);
					});
				}
				else
				{
					anosecurekit::seal_file_with_password(options.input, options.output, options.password, options.aad);
				}
			}
			else if (is_arg(argv[1], "open-file-password"))
			{
				...
				{
					anosecurekit::open_file_with_password(options.input, options.output, options.password, options.aad);
				}
			}
			else
			{
