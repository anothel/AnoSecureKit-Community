...
`docs/NEXT_WORK_QUEUE.md` records the canonical active work queue; `docs/ROADMAP.md` is a compatibility entry point.
...
- C++20 compiler.
- CMake 3.20 or newer.
- OpenSSL 3.x, version 3.0 or newer, for the shipped Community production profile.
...
cmake --build build --config Release --target release-notes-check
cmake --build build --config Release --target spdx-check
cmake --build build --config Release --target legacy-name-check
cmake --build build --config Release --target document-alignment-check
...
C++ sources against `.clang-format`. `release-notes-check` validates and extracts
exactly the section matching the CMake project version. `spdx-check` verifies
MPL-2.0 SPDX headers, `legacy-name-check` rejects stale old-name or Apache-era
identifiers outside allowlisted release-note context, `document-alignment-check`
prevents the static site and compatibility roadmap from drifting from canonical
current documents, and `cli-docs-check` compares the built CLI's
...
`project(... VERSION 0.4.1)` should be released with tag `v0.4.1`.
...
[docs/RELEASE_NOTES.md](docs/RELEASE_NOTES.md). The release workflow extracts
exactly the matching `## vX.Y.Z` section and passes the reviewed file to both
`gh release create` and `gh release edit` with `--notes-file`. Generated release
notes are not used. Publication evidence must compare the resulting GitHub
Release body with that canonical section and must not claim exact parity when
they differ.
...
FetchContent_Declare(
  anosecurekit
  URL https://github.com/anothel/AnoSecureKit-Community/releases/download/v0.4.1/anosecurekit-0.4.1-source.tar.gz
...
AnoSecureKit Community uses OpenSSL 3.x as its only shipped production provider. It does not:
...
post-v0.4.0 queue. [`docs/ROADMAP.md`](docs/ROADMAP.md) is retained only as a
compatibility entry point and must not become a second queue. Completed work is kept in Git history or release
