...
allowlisted release-note context, `document-alignment-check` prevents the static
site and compatibility roadmap from drifting from canonical current documents,
and `cli-docs-check` compares the built CLI's
...
[docs/RELEASE_NOTES.md](docs/RELEASE_NOTES.md). Compare the published GitHub
Release body with the matching version section and retain any difference as
publication evidence; do not claim exact parity when the bodies differ.
...
FetchContent_Declare(
  anosecurekit
  URL https://github.com/anothel/AnoSecureKit-Community/releases/download/v0.4.0/anosecurekit-0.4.0-source.tar.gz
